...
SELECT idSolicitud FROM solicitud WHERE idUsuario='6';
...
SELECT u.idUsuario,u.nombre,u.apellido,u.correoElectronico, 
	CONCAT(CAST(ROUND((Sum(p.fechaDeCancelacion)-SUM(p.fechaInicio))/COUNT(*)) AS CHAR),' Días') as 'Días en pagar un trámite'
	From pago p  
...
SELECT nombre, apellido,
  (SELECT COUNT(*)
   ...
   AND s.estado = 'en proceso') AS 'Solicitudes en Proceso'
